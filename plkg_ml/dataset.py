...
class csi_rssi_dataset(base_Dataset):
    def __init__(self, data: np.array):
        self.uavdata = torch.from_numpy(data[0,:,:52]).float()
        self.iotdata = torch.from_numpy(data[1,:,1:52]).float()
        self.length = len(self.uavdata)
        super().__init__(data)
    def __getitem__(self, index):
...
# our approach
#STA,AP,STA
class csi_rssi_cnn_dataset(base_Dataset):
    def __init__(self, data: np.array):
        self.uavdata = torch.from_numpy(data[0,:,:52]).float()
        self.iotdata = torch.from_numpy(data[1,:,1:52]).float()
        self.length = len(self.uavdata)-1
...
#csi cnn speed approach
class csi_cnn_speed_dataset(base_Dataset):
    def __init__(self, data: np.array):
...
# rssi cnn speed approach
class csi_rssi_cnn_speed_dataset(base_Dataset):
    def __init__(self, data: np.array):
        self.uavdata = torch.from_numpy(data[0,:,:53]).float()
        self.iotdata = torch.from_numpy(data[1,:,1:52]).float()
        self.length = len(self.uavdata)-1
        super().__init__(data)
    def __getitem__(self, index):
